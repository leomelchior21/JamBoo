...
test('detects math categories in three languages', () => {
  assert.equal(isMathCategory('Multiplication tables'), true);
  assert.equal(isMathCategory('Frações e decimais'), true);
  assert.equal(isMathCategory('Geometría'), true);
  assert.equal(isMathCategory('The French Revolution'), false);
});

test('generates deterministic multiple-choice questions with code-computed answers', () => {
  const slot = {
    slotId: '0-0',
  ...
  assert.equal(question.a, first.evidence.answer);
...
test('generates open and drawing math questions with code-checked answers', () => {
  ...
  assert.equal(open.question.a, open.evidence.answer);
  ...
  assert.ok(drawing.question.a.includes(drawing.evidence.answer));
});

test('easy slots ask real-world contexts instead of bare expressions', () => {
  const slot = {
    slotId: '0-1',
    category: 'Addition',
    categoryIndex: 0,
  ...
  const question = generateMathQuestion(slot, { language: 'English', seed: 'seed-context' }).question;
